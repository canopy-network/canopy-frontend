...
export default function LpEarningsHistorySheet({
  open,
  onOpenChange,
  lpEarningsHistory = [],
  lpPositions = [],
  ...
  const totalEarnings = lpEarningsHistory.reduce(
        ...
        <div className="mt-6 space-y-6">
          ...
          <div className="p-4 bg-muted rounded-lg">
            ...
            <div className="text-xs text-muted-foreground mt-1">
              From {lpPositions.length} active position
              {lpPositions.length !== 1 ? "s" : ""}
            </div>
          </div>
            ...
            <div className="space-y-6 pr-4">
              {groupedByDate.length > 0 ? (
                groupedByDate.map((day, dayIndex) => {
                  const dayTotal = day.earnings.reduce(
                    (sum, earning) => sum + earning.amount,
                    0
                  );
                      ...
                      <div className="space-y-2">
                        {day.earnings.map((earning) => {
                          const pool = available_pools.find(
                            (p) => p.id === earning.poolId
                          );
                          const tokenBData = tokens.find(
                            (t) => t.symbol === earning.tokenB
                          );
