...
function TokenAvatar({ symbol, color, size = 20 }: TokenAvatarProps) {
  if (symbol === "CNPY") {
...
export default function YourPositionCard({
  ...
  const tokenAData = tokensData.find((t) => t.symbol === pool.tokenA) as
    | Token
    | undefined;
  const tokenBData = tokensData.find((t) => t.symbol === pool.tokenB) as
    | Token
    | undefined;
        ...
        <div className="pt-3 border-t border-border space-y-2">
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2">
              <TokenAvatar
                symbol={pool.tokenA}
                color={tokenAData?.brandColor}
                size={20}
              />
              <span className="text-muted-foreground">{pool.tokenA}</span>
            </div>
            <span className="font-medium">
              {position.tokenAAmount.toLocaleString()}
            </span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-2">
            ...
            <span className="font-medium">
              {position.tokenBAmount.toLocaleString()}
            </span>
          </div>
        </div>
