...
import type { WalletEventBase, WalletEventsHistoryParams } from "@/types/wallet-events";

export const walletEventsKeys = {
  all: ["wallet-events"] as const,
  history: (addresses?: string[], filters?: Partial<WalletEventsHistoryParams>) =>
...
export function useWalletEventsHistory(params: WalletEventsHistoryParams = {}, options?: UseWalletEventsOptions) {
  const addresses = params.address ? [params.address] : params.addresses ?? [];

  const enabled = options?.enabled ?? (addresses.length > 0 || !!params.address);
