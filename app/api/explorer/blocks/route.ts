 ...
 * 
 * Returns a paginated list of recent blocks across the network.
 * 
 * Query Parameters:
 * - chain_id (uint64, optional) - Filter by specific chain ID (default: 1 for root chain)
 * - limit (integer, optional) - Number of blocks per page (default: 20, max: 100)
...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${API_BASE_URL}/api/v1/explorer/blocks${
    ...
    console.error("[API /api/explorer/blocks] Error:", error);
    return NextResponse.json(
      {
        error: {
          code: "INTERNAL_ERROR",
